...
export function JourneyRail({ activeIndex, onCheckout }: { activeIndex: number; onCheckout: (p: PlanId) => void }) {
  ...
  return (
    <div className="sticky top-0 z-40 border-b border-white/10 bg-[#07060d]/85 backdrop-blur-xl">
      <div className="mx-auto max-w-5xl px-5 py-3">
        ...
        <div className="mt-2.5 flex items-center gap-3">
          <div className="h-1 flex-1 overflow-hidden rounded-full bg-white/10">
            <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-sky-400 transition-all duration-700" style={{ width: `${pct}%` }} />
          </div>
          <span className="shrink-0 text-[11px] font-medium text-fuchsia-300/80">{inspiring}</span>
        </div>
      </div>
    </div>
  );
...
function Node({ milestone, state, onClick }: { milestone: Milestone; state: NodeState; onClick: () => void }) {
  ...
  const circle: Record<NodeState, string> = {
    done: "bg-gradient-to-br from-indigo-500/70 to-fuchsia-500/70 text-white",
    current: "bg-gradient-to-br from-indigo-500 to-fuchsia-500 text-white ring-2 ring-fuchsia-400/40 ring-offset-2 ring-offset-[#07060d]",
    next: "bg-white/[0.06] text-white/80 ring-1 ring-fuchsia-400/40",
    locked: "bg-white/[0.04] text-white/35",
  };
  ...
  return (
    <button
      ...
      className={`group flex items-center gap-2 rounded-xl px-2 py-1 transition ${clickable ? "cursor-pointer hover:bg-white/5" : "cursor-default"}`}
    >
      <span className={`relative grid h-8 w-8 shrink-0 place-items-center rounded-full text-sm transition ${circle[state]}`}>
        ...
        {state === "current" && <span className="absolute inset-0 animate-ping rounded-full bg-fuchsia-400/30" />}
      </span>
      <span className="flex flex-col text-left leading-tight">
        <span className={`text-xs font-semibold ${labelCls[state]}`}>
          {milestone.label}
          {state === "next" && <span className="ml-1.5 rounded-full bg-fuchsia-500/20 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide text-fuchsia-300">Next</span>}
        </span>
        {showTease && <span className="hidden text-[10px] text-white/40 sm:block">{milestone.tease}</span>}
      </span>
    </button>
  );
}
...
function Connector({ done }: { done: boolean }) {
  return (
    <span className="relative h-px w-5 overflow-hidden rounded-full bg-white/12 sm:w-8">
      {done && <span className="absolute inset-0 bg-gradient-to-r from-indigo-400 to-fuchsia-400" />}
    </span>
  );
}
