    ...
    <div className="animate-in pt-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          {favs.size > 0 && <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-sm text-white/60">♥ {favs.size} saved</span>}
          <button onClick={onMore} className="rounded-xl border border-white/15 px-4 py-2 text-sm font-medium transition hover:bg-white/5">↻ Regenerate</button>
        </div>
      </div>
...
function NameCard({ idea, fav, onFav, delay }: { idea: NameIdea; fav: boolean; onFav: () => void; delay: number }) {
  return (
    <div className="group glass animate-in relative rounded-2xl p-5 transition hover:border-white/20" style={{ animationDelay: `${delay * 50}ms` }}>
      <button onClick={onFav} className={`absolute right-4 top-4 text-xl transition ${fav ? "text-fuchsia-400" : "text-white/20 hover:text-white/50"}`} aria-label="Save name">
        {fav ? "♥" : "♡"}
      </button>
      ...
      <p className="mt-2 text-sm text-white/55">{idea.tagline}</p>

      <div className="mt-4 flex flex-wrap items-center gap-2">
        <DomainBadge idea={idea} />
        <span className="rounded-md border border-white/10 px-2 py-0.5 text-xs text-white/40">{TYPE_META[idea.type].label}</span>
      </div>

      <AxisBars idea={idea} />
      <p className="mt-3 text-xs leading-relaxed text-white/35">{idea.rationale}</p>
    </div>
  );
}
      ...
      {axes.map((a) => (
        <div key={a.k} title={`${a.k}: ${a.v}`}>
          <div className="h-1 w-full overflow-hidden rounded-full bg-white/10">
            <div className="h-full rounded-full bg-gradient-to-r from-indigo-400 to-fuchsia-400" style={{ width: `${a.v}%` }} />
          </div>
          <span className="mt-1 block text-[10px] text-white/35">{a.k}</span>
        </div>
      ))}
    </div>
  );
...
function ScorePill({ score }: { score: number }) {
  const color = score >= 85 ? "text-emerald-400" : score >= 75 ? "text-sky-400" : "text-white/50";
...
function DomainBadge({ idea }: { idea: NameIdea }) {
  if (idea.premiumDomain) return <span className="rounded-md bg-amber-500/10 px-2 py-0.5 text-xs font-medium text-amber-300">◆ {idea.domain} · premium</span>;
  if (idea.domainAvailable) return <span className="rounded-md bg-emerald-500/10 px-2 py-0.5 text-xs font-medium text-emerald-300">● {idea.domain} available</span>;
      ...
      <div className="pointer-events-none grid select-none gap-4 opacity-50 blur-[6px] sm:grid-cols-2">
        {teaser.map((n) => (
          <div key={n.name} className="glass rounded-2xl p-5">
            ...
            <p className="mt-2 text-sm text-white/50">{n.tagline}</p>
          </div>
        ))}
      </div>

      <div className="absolute inset-0 flex items-center justify-center">
        <div className="glass w-full max-w-md rounded-3xl border-fuchsia-400/30 p-7 text-center shadow-2xl shadow-fuchsia-500/10">
