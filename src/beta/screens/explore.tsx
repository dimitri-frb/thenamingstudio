    ...
    <>
      <div className="bbody" style={{ paddingBottom: 26 }}>
        <div style={{ display: "flex", alignItems: "baseline", gap: 14, flexWrap: "wrap" }}>
          ...
          <span style={{ fontSize: 12.5, color: "var(--ink-3)" }}>Click any word to explore its own world · save the ones you like →</span>
        </div>
        <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
          <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 18 }}>
            <div className="bconcept-banner">
              <span className="bconcept-k">Concept</span>
              <span className="bconcept-w">{world}</span>
              <span className="bconcept-q">“{brief.does || "An AI naming studio that finds a brand name with a strategist's rigor — in minutes, not months."}”</span>
            </div>
            <p style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--ink-3)", margin: 0 }}>Words that fit the brief</p>
            {busy && !groups.length ? (
                      ...
                      return (
                        <div key={w.w + i} className="bword" onClick={() => explore(w.w)}>
                          ...
                          <div className="bword-foot">
                            <span className="bword-explore">Explore →</span>
          ...
          <div style={{ width: 208, flex: "0 0 auto" }}>
            <div className="bsaved">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                {saved.map((s) => (
                  <div key={s.w} className="bsaved-row"><span style={{ color: "var(--accent)", fontSize: 11 }}>★</span>{s.w}
                    <span style={{ marginLeft: "auto", color: "var(--ink-4)", cursor: "pointer" }} onClick={() => toggle(s.w)}>×</span>
                  </div>
                ))}
              ...
              <button className="bbtn" style={{ width: "100%", marginTop: 14, justifyContent: "center" }}
                disabled={!saved.length} onClick={onDone}>Shape into names →</button>
            </div>
          </div>
        </div>
      </div>
...
export function BetaNameIdeas({ brief, saved, shortlist, setShortlist, initialRows, onNext }: {
  ...
  const sorted = [...ideas].sort((a, b) => (b.score || 0) - (a.score || 0)).slice(0, 6);

  return (
    <>
      <div className="bbody">
        <BHead eyebrow="The names" title={<>From saved words to real names.</>}
          sub="Six candidates, built from the words you saved and shaped by your strategy. Shortlist the ones worth comparing." />
        {busy && !ideas.length ? (
