...
function buildPrompt(phase: string, brief: any, payload: any): string {
  ...
  switch (phase) {
    ...
    case "explore":
      return `${SYS}\n\nBRIEF:\n${B}\n\nTERRITORY TO EXPLORE IN DEPTH:\n${JSON.stringify(payload.concept)}\n\nTASK: Turn this one direction into an inspiring, explicit "world" the founder can react to — like sketching the brand so it gets clearer. Talk to their gut and creativity, not just logic. Give:\n- essence: a punchy one-line soul of this direction (a few words).\n- story: one vivid, warm sentence on why this direction fits THIS brand and audience.\n- quotes: exactly 2 manifesto-style lines in the brand's VOICE — how it would actually talk (short, evocative, NOT generic marketing).\n- brands: exactly 4 real, well-known brands that already live in this world; for each give name + a 4-8 word "why" describing the shared vibe (do NOT use the founder's own company).\n- angles: exactly 4 distinct sub-territories / narrower takes on this direction; each with a short title (1-3 words) and a 6-12 word note on what owning that angle would mean.\nSCHEMA: {"title":"","essence":"","story":"","quotes":["",""],"brands":[{"name":"","why":""}],"angles":[{"title":"","note":""}]}`;
    case "names":
      return `${SYS}\n\nBRIEF:\n${B}\n\nTHE BRAND SO FAR (what the founder kept while exploring — treat this as their taste/soul):\n${JSON.stringify(payload.sketch)}\n\nTASK: Generate exactly 24 candidate BRAND NAMES that feel like they belong to THIS brand sketch — drawn from the kept concepts, manifesto quotes (voice), reference brands (vibe) and angles. Use a mix of the chosen lanes. Avoid names obviously owned by famous companies; prefer ownable, brandable options. For each: name, type (one of [descriptive, suggestive, compound, invented, abstract, founder, acronym, evocative, geographic, playful]), a one-line human rationale tying it back to the sketch, and a 0-100 "pulse" score for overall brand strength.\nSCHEMA: {"names":[{"name":"","type":"","rationale":"","score":0}]}`;
    case "suggest":
