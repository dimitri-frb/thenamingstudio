...
function buildPrompt(phase: string, brief: any, payload: any): string {
  ...
  switch (phase) {
    ...
    case "compare":
      return `${SYS}\n\nBRIEF:\n${B}\n\nSHORTLIST TO COMPARE:\n${JSON.stringify(payload.names)}\n\nTASK: Build a decision comparison for these names. For EACH name: score 1-5 on intuitive, visual, sound, emotional and give total (sum, out of 20). domains = a best-GUESS availability read on 3 TLDs, .com ALWAYS FIRST, then .io and .co (available is your honest estimate true/false; these are guesses, not live lookups). inpi = your best estimate (true if it looks clear to register as a French/EU trademark, false if there's a plausible earlier mark) and inpiNote = one short sentence explaining (mention it's an estimate). verdict = one short, human sentence. Finally choose the single best "recommended" name and write "why" as one warm, human, first-person paragraph from the studio (not AI-sounding).\nSCHEMA: {"rows":[{"name":"","intuitive":0,"visual":0,"sound":0,"emotional":0,"total":0,"domains":[{"tld":".com","available":true},{"tld":".io","available":true},{"tld":".co","available":true}],"inpi":true,"inpiNote":"","verdict":""}],"recommended":"","why":""}`;
    default:
