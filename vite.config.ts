...
function buildPrompt(phase: string, brief: any, payload: any): string {
  ...
  switch (phase) {
    ...
    case "explore":
      return `${SYS}\n\nBRIEF:\n${B}\n\nTERRITORY TO EXPLORE IN DEPTH:\n${JSON.stringify(payload.concept)}\n\nTASK: Turn this one naming territory into a rich, explorable "world" for the founder to dig into. Give:\n- story: one vivid sentence on why this direction fits THIS brand (warm, human).\n- mood: 5 single adjectives that capture the feeling.\n- palette: 4 hex colors that evoke the mood (varied, tasteful).\n- motif: one short visual motif (a few words or an emoji) — what you'd see/draw.\n- references: 3 short cultural/brand touchstones in this territory.\n- samples: 3 example brand names already living in this world (ownable, on-brief, not famous-company names).\n- words: 8 seed WORDS to mine. For EACH word give a 3-6 word note (its meaning/why it fits) and 4 branches — related words to go deeper, each tagged kind = one of [synonym, metaphor, foreign, sound].\nSCHEMA: {"title":"","story":"","mood":["","","","",""],"palette":["#","#","#","#"],"motif":"","references":["","",""],"samples":["","",""],"words":[{"word":"","note":"","branches":[{"word":"","kind":"synonym"}]}]}`;
    case "names":
